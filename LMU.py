...
class LMUCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, memory_size, theta, learn_a = False, learn_b = False):
        ...
        if learn_a: # updated during training
            self.A = nn.Parameter(A)
        else:       # fixed during training
...
class LMUModel(nn.Module):
    def __init__(self, 
        ...
        if FFT == False:
            self.lmu = LMU(self.input_size, self.hidden_size, self.memory_size, theta, learn_a, learn_b)
        else:
            self.lmufft1 = LMUFFT(self.input_size, self.hidden_size, self.memory_size, self.seq_len, theta)
            self.lmufft2 = LMUFFT(self.hidden_size, self.hidden_size//2, self.memory_size, self.seq_len, theta)
            self.lmufft3 = LMUFFT(self.hidden_size//2, self.hidden_size//4, self.memory_size, self.seq_len, theta)
            self.lmufft4 = LMUFFT(self.hidden_size//4, self.hidden_size//8, self.memory_size, self.seq_len, theta)
            # self.lmufft5 = LMUFFT(self.hidden_size//8, self.hidden_size//16, self.memory_size, self.seq_len, theta)
            # self.lmufft6 = LMUFFT(self.hidden_size//16, self.hidden_size//32, self.memory_size, self.seq_len, theta)

        self.layernorm = nn.LayerNorm([self.seq_len, self.hidden_size]) 
        self.layernorm1 = nn.LayerNorm([self.seq_len, self.hidden_size//2]) 
        self.layernorm2 = nn.LayerNorm([self.seq_len, self.hidden_size//4]) 
        # self.layernorm3 = nn.LayerNorm([self.seq_len, self.hidden_size//8]) 
        # self.layernorm4 = nn.LayerNorm([self.seq_len, self.hidden_size//16]) 
        # self.layernorm = nn.BatchNorm1d(self.hidden_size) 
        # self.layernorm1 = nn.BatchNorm1d(self.hidden_size//2) 
        # self.layernorm2 = nn.BatchNorm1d(self.hidden_size//4) 
        self.fc = nn.Linear(self.hidden_size//8, self.output_dim)
        self.dropout = nn.Dropout(p=self.dropout_rate)
        self.activation = nn.ReLU()

    def forward(self, x):    
        if self.FFT == False:
            o_n, (h_n, m_n) = self.lmu(x)
        else:
            o_n, h_n = self.lmufft1(x) 
            o_n = self.layernorm(o_n)
            o_n = self.dropout(o_n)
            o_n, h_n = self.lmufft2(o_n)
            o_n = self.layernorm1(o_n)
            o_n = self.dropout(o_n)
            o_n, h_n = self.lmufft3(o_n)
            o_n = self.layernorm2(o_n)
            o_n = self.dropout(o_n)
            o_n, h_n = self.lmufft4(o_n)
            # o_n = self.layernorm3(o_n)
            # o_n = self.dropout(o_n)
            # o_n, h_n = self.lmufft5(o_n)
            # o_n = self.layernorm4(o_n)
            # o_n = self.dropout(o_n)
            # o_n, h_n = self.lmufft6(o_n)
...
if __name__ == "__main__":
    ...
    N_t = 2376 # number of timesteps
    N_h = 512 # dimension of the hidden state
    N_m = 512 # dimension of the memory
    ...
    temp = torch.rand((N_b, N_t, N_x)).to(DEVICE)
    out = model(temp)
